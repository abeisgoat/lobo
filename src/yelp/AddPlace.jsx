...
const mbxStaticClient = mbxStatic({
  accessToken:
    "pk.eyJ1IjoiYWJlaXNncmVhdCIsImEiOiJrOEhRekZVIn0.9KVGXDIo5bd6pghddAd44Q"
});

const forwardGeocodingURL = query =>
  `https://api.mapbox.com/geocoding/v5/mapbox.places/${query}.json?access_token=pk.eyJ1IjoiYWJlaXNncmVhdCIsImEiOiJrOEhRekZVIn0.9KVGXDIo5bd6pghddAd44Q&types=poi&bbox=-98.02276650312514%2C30.057825160616275%2C-97.42401025743476%2C30.527207193138608`;
...
export const add_place_data = (state = defaultState, action) => {
  switch (action.type) {
    ...
    case "SET_ADD_PLACE_QUERY_RESULTS":
      return {
        ...state,
        query_results: action.query_results.map(feature => ({
          name: feature.text,
          id: feature.id.replace(".", "_"),
...
function AddPlace(props) {
  ...
  return (
    <div className={`overlay ${props.active && "visible"}`}>
      <div className="walls">
        <div className="wall">
          {!props.place.id && (
            <div className="tray">
              ...
              <input
                type="text"
                placeholder="Name"
                onKeyUp={e => store.dispatch(fetchQueryResults(e.target.value))}
              />
                ...
                <ul>
                  {props.results.map(result => (
                    <li
                      hoverable="true"
                      key={result.id}
                      onClick={() => props.setAddPlacePlace(result)}
                    >
                      {result.name}
                      <br />
          ...
          {props.place.id && (
            <div className="tray">
              <BarImageH2
                image={mapURL(props.place.point)}
                title={props.place.name}
              />
              <input type="text" defaultValue={props.place.categories} />
              <textarea
                defaultValue={props.place.comments}
              ...
              <div className="bar" half_padded="true">
                ...
                <button
                  onClick={() => add(props.place) && props.closeAddPlace()}
                >
                  Save
                </button>
              </div>
